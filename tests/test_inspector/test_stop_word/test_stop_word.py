...
class TestStopWordChecker:
    ...
    async def test_stopword(self, test_case: StopWordTestCase, telegram_id, load_stop_word_checker, mocker):
        ...
        with test_case.expected:
            return_message = await stop_word_checker.monitor_stop_words(
                telegram_id=telegram_id, response_message=test_case.income_message
            )
