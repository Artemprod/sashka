...
class ResearchWordStopper(BaseConfig):
    stop_words: List[str] = Field(default_factory=lambda: BASE_WORDS.copy())
...
class ResearchPingator(BaseConfig):
    ...
    max_pings_messages: int = Field(default=4, validation_alias="PINGATOR_MAX_PINGS_MESSAGES")
    ping_attempts_multiplier: int = Field(default=2, validation_alias="PINGATOR_PING_ATTEMPT_MULTIPLIER")
    ping_interval: int = Field(default=10, validation_alias="PINGATOR_PING_INTERVAL_CHEKING")
