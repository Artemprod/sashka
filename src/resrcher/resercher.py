...
class TelegramResearcher(BaseResearcher):
    ...
    #     research_dto = self._create_research_dto(owner, telegram_client)
    #     db_research = await self._save_new_research(research_dto)
    #     await self.database_repository.status_repo.research_status.change_research_status(research_id=db_research.research_id, status=ResearchStatusEnum.WAIT)
    #
    #     # Собрать информацию о пользователях и добавить их в исследование
    #     users_dto = await self._collect_users_information(self.research.user_ids)
    #     db_users = await self._add_users_to_research(users_dto)
    #     await self.database_repository.status_repo.user_status.change_status_group_of_user(user_group=[user.tg_user_id for user in db_users], status=UserStatusEnum.WAIT)
    #     # Связать пользователей с исследованием
    #     await self._bind_users_to_research(db_users, db_research.research_id)
    ...
    # def _create_research_dto(self, owner: Any,
    ...
    #
    # # TODO Не забывай что тут возвращаются объекты модели данных а не DTO допиши конвертацию
    # async def _add_users_to_research(self, users_dto: List[UserDTO]) -> List[Any]:
    #     return await self.database_repository.user_in_research_repo.short.add_many_users(users_dto)
    #
    # async def _bind_users_to_research(self, db_users: List[Any], research_id: int) -> None:
    #     for db_user in db_users:
    ...
    #         print(f"____________Текущий статус исследования: {research_status.status_name}, Ожидаемый статус: {ResearchStatusEnum.IN_PROGRESS}")
    #
    #         if research_status != ResearchStatusEnum.IN_PROGRESS.value:
    #             print(f"_____________Завершил исследование по изменению статуса {research_status.status_name}")
    #             event.set()
    #             break
    #
    #         await asyncio.sleep(self.settings.get('delay_is_users_over', 10))
    #     else:
    #         print(f"_____________Завершил исследование по изменению события ")
