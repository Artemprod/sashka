...
async def command_dialog_start(
    body: str, msg: NatsMessage, context=Context(), command_data=Depends(get_command_start_dialog_data_from_headers)
):
...
async def command_user_ping(
    ...
    try:
        # Обработка сообщения с использованием TelegramCommunicator
        await communicator.ping_user(
            user=command_ping_data.user,
